...
// Logging for debugging (only in dev)
const logRequest = (method: string, url: string) => {
  if (__DEV__) console.log(`[API] ${method} ${url}`);
};

// Helper function to get auth token from storage (platform-aware)
...
export const jobsAPI = {
  ...
  // Start work on a job (student)
  async startWork(jobId: number, studentId: number) {
    const token = await getAuthToken();
    const response = await fetch(`${API_BASE_URL}/jobs/${jobId}/start-work`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => ({ error: 'Failed to start work' }));
      throw new Error(err.error || 'Failed to start work');
    }
    return response.json();
  },

  // Stop work and submit client signature (student)
  async stopWork(jobId: number, studentId: number, clientSignature: string) {
    const token = await getAuthToken();
    const response = await fetch(`${API_BASE_URL}/jobs/${jobId}/stop-work`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => ({ error: 'Failed to stop work' }));
      throw new Error(err.error || 'Failed to stop work');
    }
    return response.json();
  },

  // Get current work session state (for timer restore on refresh)
  async getWorkSession(jobId: number, studentId: number) {
    const token = await getAuthToken();
    const response = await fetch(
      `${API_BASE_URL}/jobs/${jobId}/work-session?student_id=${studentId}`,
      {
        headers: token ? { 'Authorization': `Bearer ${token}` } : {},
      }
    );
    if (!response.ok) {
      const err = await response.json().catch(() => ({ error: 'Failed to get work session' }));
      throw new Error(err.error || 'Failed to get work session');
    }
    return response.json();
  },
...
export const authAPI = {
  ...
  async logout() {
    const keys = ['authToken', 'studentId', 'clientId', 'user', 'token'];
    if (Platform.OS === 'web' && typeof window !== 'undefined') {
