import { StyleSheet, TouchableOpacity, ScrollView, Platform, StatusBar, Alert, TextInput, Modal } from "react-native";
...
export default function TabOneScreen() {
  ...
  const handleCreateAdmin = async () => {
    if (!adminEmail || !adminPassword) {
      setAdminMsg("Fill in both fields");
      return;
    }
    try {
      ...
      setAdminMsg(`Admin created! (${data.user.email})`);
      setAdminEmail("");
      setAdminPassword("");
    } catch (err: any) {
  ...
  return (
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#FFFFFF" }} edges={['top']}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.container}>
        ...
        {/* TEMP: CREATE ADMIN BUTTON */}
        <View style={{ alignItems: "center", paddingVertical: 20 }}>
          <TouchableOpacity
            onPress={() => setShowAdminModal(true)}
            style={{ paddingVertical: 10, paddingHorizontal: 24, backgroundColor: "#374151", borderRadius: 8 }}
          >
            <Text style={{ color: "#9CA3AF", fontSize: 12, fontWeight: "600" }}>Create Admin (dev)</Text>
          </TouchableOpacity>
        </View>
        ...
        <Modal visible={showAdminModal} transparent animationType="fade">
          <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "center", alignItems: "center" }}>
            <View style={{ backgroundColor: "#fff", borderRadius: 16, padding: 24, width: "90%", maxWidth: 400 }}>
              <Text style={{ fontSize: 20, fontWeight: "700", marginBottom: 16, color: "#041316" }}>Create Admin Account</Text>
              <TextInput
                ...
                style={{ borderWidth: 1, borderColor: "#E1E7EB", borderRadius: 8, padding: 12, marginBottom: 12, fontSize: 14, color: "#041316" }}
              />
              <TextInput
                ...
                style={{ borderWidth: 1, borderColor: "#E1E7EB", borderRadius: 8, padding: 12, marginBottom: 12, fontSize: 14, color: "#041316" }}
              />
              {adminMsg ? <Text style={{ color: adminMsg.includes("created") ? "#176B51" : "#DC2626", marginBottom: 12, fontSize: 13 }}>{adminMsg}</Text> : null}
              <View style={{ flexDirection: "row", gap: 12 }}>
                <TouchableOpacity
                  onPress={() => { setShowAdminModal(false); setAdminMsg(""); }}
                  style={{ flex: 1, paddingVertical: 12, borderRadius: 8, borderWidth: 1, borderColor: "#E1E7EB", alignItems: "center" }}
                >
                  <Text style={{ color: "#5D6B73", fontWeight: "600" }}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={handleCreateAdmin}
                  style={{ flex: 1, paddingVertical: 12, borderRadius: 8, backgroundColor: "#176B51", alignItems: "center" }}
                >
