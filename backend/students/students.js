 ...
 * Update student profile info (protected via server.js JWT mount)
 */
 ...
 * Apply for a job (protected via server.js JWT mount)
 */
router.post("/:studentId/apply", async (req, res) => {
  try {
    const studentId = parseInt(req.params.studentId);
    ...
    // Check student verification status — only verified students can apply
    ...
    if (studentProfile.verification_status !== 'verified') {
      return res.status(403).json({
        error: 'Your profile must be verified by an admin before you can apply to jobs',
 ...
 */
router.patch("/:studentId/applications/:applicationId", async (req, res) => {
  try {
    const studentId = parseInt(req.params.studentId);
