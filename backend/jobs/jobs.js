 ...
 */
if (upload) {
  router.post("/upload-image", verifyJwt, upload.single("image"), async (req, res) => {
    try {
      const file = req.file;
      if (!file) {
...
router.post("/draft", verifyJwt, async (req, res) => {
  try {
    const {
      client_id,
      category_id,
 ...
 */
router.get("/:jobId/applicants", verifyJwt, async (req, res) => {
  try {
 ...
 */
router.patch("/:jobId/applicants/:applicationId", verifyJwt, async (req, res) => {
  try {
    const jobId = parseInt(req.params.jobId, 10);
 ...
 */
router.delete("/:jobId", verifyJwt, async (req, res) => {
  try {
    const jobId = parseInt(req.params.jobId, 10);
 ...
 * Update job status
 */
router.patch("/:jobId/status", verifyJwt, async (req, res) => {
  try {
 ...
 * Body: { student_id }
 */
router.post("/:jobId/mark-in-progress", verifyJwt, async (req, res) => {
  try {
    const jobId = parseInt(req.params.jobId, 10);
 ...
 * Body: { student_id }
 */
router.post("/:jobId/mark-completed", verifyJwt, async (req, res) => {
  try {
    const jobId = parseInt(req.params.jobId, 10);
    const { student_id } = req.body;
 ...
 */
router.get("/:jobId/payment-info", verifyJwt, async (req, res) => {
  try {
    const jobId = parseInt(req.params.jobId, 10);
...
/**
 * POST /jobs/:jobId/start-work
 * Student starts working on an accepted job.
 * Only allowed within 30 minutes before (or after) the scheduled start_time.
 * Records actual_start_time and transitions job to 'in_progress'.
 * Body: { student_id }
 */
router.post("/:jobId/start-work", verifyJwt, async (req, res) => {
  try {
    const jobId = parseInt(req.params.jobId, 10);
    const { student_id } = req.body;
    ...
    // Verify student has accepted application
    const { data: application, error: appError } = await supabase
      .from("job_applications")
      .select("id, status, actual_start_time")
      .eq("job_id", jobId)
      .eq("student_id", student_id)
      .eq("status", "accepted")
      .maybeSingle();
    ...
    if (application.actual_start_time) {
      return res.status(400).json({
        error: "Work has already been started for this job",
      });
    }

    // Get job to check start_time
    const { data: job, error: jobError } = await supabase
      .from("jobs")
    ...
    // Allow start within 30 minutes before scheduled time (or anytime after)
    const now = new Date();
    ...
    if (minutesUntilStart > 30) {
      return res.status(400).json({
        error: `Too early to start. Job begins in ${Math.round(minutesUntilStart)} minutes. You can start up to 30 minutes before.`,
      });
    }

    const actualStartTime = now.toISOString();

    // Record actual_start_time on application
    const { error: updateAppError } = await supabase
      .from("job_applications")
      .update({ actual_start_time: actualStartTime })
...
/**
 * POST /jobs/:jobId/stop-work
 * Student stops work and client signs. Completes the job.
 * Body: { student_id, client_signature (base64) }
 */
router.post("/:jobId/stop-work", verifyJwt, async (req, res) => {
  try {
    const jobId = parseInt(req.params.jobId, 10);
    const { student_id, client_signature } = req.body;

    if (!jobId || !student_id || !client_signature) {
      return res
        .status(400)
        .json({ error: "job_id, student_id, and client_signature are required" });
    }

    // Verify student has accepted application with actual_start_time
    const { data: application, error: appError } = await supabase
      .from("job_applications")
      .select("id, status, actual_start_time")
      .eq("job_id", jobId)
    ...
    if (appError || !application) {
      return res
        .status(403)
        .json({ error: "No accepted application found for this job" });
    }

    if (!application.actual_start_time) {
      return res
        .status(400)
        .json({ error: "Work has not been started yet" });
    }

    const actualEndTime = new Date().toISOString();

    // Calculate duration in minutes
    const startMs = new Date(application.actual_start_time).getTime();
    const endMs = new Date(actualEndTime).getTime();
    const durationMinutes = Math.round((endMs - startMs) / 60000);

    // Update application with end time, signature, and mark completed
    const { error: updateAppError } = await supabase
      .from("job_applications")
      .update({
    ...
    res.json({
      ...
      application: {
        id: application.id,
        actual_start_time: application.actual_start_time,
        actual_end_time: actualEndTime,
    ...
    res.status(500).json({ error: "Failed to stop work" });
  }
});

/**
 * GET /jobs/:jobId/work-session
 * Get work session state for the accepted student (used to restore timer on refresh)
 * Query: ?student_id=123
 */
router.get("/:jobId/work-session", verifyJwt, async (req, res) => {
  try {
    const jobId = parseInt(req.params.jobId, 10);
    ...
    // Get the accepted/completed application
    const { data: application, error: appError } = await supabase
      .from("job_applications")
      .select("id, status, actual_start_time, actual_end_time, client_signature, applied_at")
      .eq("job_id", jobId)
      .eq("student_id", studentId)
    ...
    console.error("Error fetching work session:", err);
